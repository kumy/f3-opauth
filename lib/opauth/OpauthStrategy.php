...
class OpauthStrategy {
	...
	public static function clientPost($url, $data = array()) {
		...
		$html .= '</form>';
		$nonce = '';
		if (class_exists('GeoKrety\\Service\\SecurityHeaders')) {
			$nonceValue = \GeoKrety\Service\SecurityHeaders::instance()->getNonce();
			$nonce = ' nonce="' . htmlspecialchars($nonceValue, ENT_QUOTES, 'UTF-8') . '"';
		}
		$html .= '<script type="text/javascript"' . $nonce . '>function postit(){ document.auth.submit(); } postit();</script>';
		$html .= '</body></html>';
		echo $html;
	}
